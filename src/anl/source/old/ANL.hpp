...
namespace anl {
    ...
    int get_nmodules();
    std::vector<VANL_Module*>::iterator get_module_chain();
    analysis_status* get_count(int index);
    //int get_module(std::string key, VANL_Module* mod);//int AnlMod(std::string key, VANL_Module* mod);
    VANL_Module* get_module(std::string key);//VANL_Module* AnlMod(std::string key);
    //VANL_Module* get_module(int index);
    ...
    int anl_end();
    int anl_body();
    int anl_ql_body(std::vector<VANL_Module*> &chain);
    int anl_ql2_body(std::vector<VANL_Module*> &chain);
    /* end ANLCROSS 0 */
