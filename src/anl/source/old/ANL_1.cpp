...
int anl::anl_define_analysis()
{
    ...
    while(true){
	int mod_num = -1;
	mod_num = com_cli::read_value<int>("What module is switched on/off:( to exit: -1)", mod_num);
...
int anl::anl_modify_param()
{
    ...
    ichoice = com_cli::read_keyword("ANL *** Modify Parameter ***", table, -1);
	...
	if( answer == "NONE" || ichain >= tablesize || ichain < 0 ) break;
	    ...
	    msec = com_cli::read_value<int>("Display interval [msec]", msec);
...
int anl::anl_book_histgram()
{
    return ANL_OK;
}
int anl::anl_talk_menu()
{
    std::vector<std::string> table = {
	"SHOW_ANALYSIS",
	"DEFINE_ANALYSIS",
	...
	ichoice = com_cli::read_keyword("ANL *** Talk Menu ****", table, 1);
	if ( (int)ichoice.size()==0 ) continue;
...
int anl::anl_read_data()
{
    ...
    // ask event parameter
    int n_rest_events = 1;
    n_rest_events = com_cli::read_value<int>("Number of events (-1=all, 0=exit)", n_rest_events);
    if(n_rest_events == 0){
    ...
    event_freq = com_cli::read_value<int>("Event number printout frequency", event_freq);    
...
int anl::anl_save_hist()
{
    return ANL_OK;
}
int anl::anl_clear_hist()
{
    return ANL_OK;
}
int anl::anl_show_status()
{
    using namespace std;
...
int anl::anl_ana_menu()
{
    ...
    while(true){
	ichoice = com_cli::read_keyword("ANLPLUS *** Analysis Menu ****", table, 1);
	if ( ichoice.size()==0 ) continue;
	std::string answer( table[ ichoice[0] ] );
...
int anl::anl_error(int level, std::string place, std::string message)
{
    if(level==0){
